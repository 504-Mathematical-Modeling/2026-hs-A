...
针对问题三，采用粗网格扫描与二分搜索相结合的策略，以 Wilson 下限作为统计认证标准，得到最低认证体积分数约为 $1.08\%$，对应约 $765$ 根 A。

针对问题四，构建 A/B 混合成本模型，A 单位体积价格约为 B 的 $21$ 倍。网格搜索（$98$ 个配置，每配置 $M=200$）表明：点估计最低成本可行方案为 $495$ 根 A 与 $358$ 个 B（$\hat p=0.935$、成本 $7.95$ 元）；以 Wilson 下限 $\ge 0.90$ 认证的最低成本方案为 $495$ 根 A 与 $477$ 个 B（下限 $0.923$、成本 $8.15$ 元），较 $765$ 根纯 A 方案节省约 $28\%$ 成本，体现长圆柱骨架与低价球体的互补作用。
...
\subsubsection{数值结果}
表~\ref{tab:q2result} 汇总四个指定体积分数。图~\ref{fig:q2curve} 给出点估计曲线，图~\ref{fig:q2ci} 给出置信区间。结果呈现明显的上升趋势，与式~\eqref{eq:scaling_law} 的渗流标度律定性一致：低填充时概率较小，超过临界阈值后急剧上升并趋于饱和。
\begin{table}[H]\centering\caption{问题二概率结果（$M=2000$）}\label{tab:q2result}\begin{tabular}{ccccc}\toprule $\phi_A$&$N_A$&$K/M$&$\hat p$&Wilson区间\\\midrule 0.50\%&354&629/2000&0.3145&[0.2945,0.3352]\\0.60\%&424&895/2000&0.4475&[0.4258,0.4694]\\0.70\%&495&1165/2000&0.5825&[0.5607,0.6039]\\0.80\%&566&1386/2000&0.6930&[0.6724,0.7128]\\0.90\%&637&1579/2000&0.7895&[0.7711,0.8068]\\1.00\%&707&1746/2000&0.8730&[0.8577,0.8869]\\\bottomrule\end{tabular}\end{table}
...
\subsubsection{机制分析}
从渗流理论角度，增加 $N_A$ 同时增大了三个效应\cite{berhan2007}：(i) 电极附近节点密度增加，增大了"种子"连通分量的起始规模；(ii) 介质间排除体积重叠概率增大，更多孤立团簇通过新边合并；(iii) 当 $N$ 足够大时，最大连通分量的相对尺寸从零突变到非零值——这正是渗流相变的特征。本题中 $\phi_A=0.50\%$ 时 $\hat p=0.3145$ 处于亚临界区附近（导通尚未成为大概率事件），$\phi_A=1.00\%$ 时 $\hat p=0.8730$ 已接近超临界区，概率曲线的 S 形形态与三维渗流标度律一致。
...
\subsubsection{结果小结}
在现有实验下（$M=2000$），$0.70\%$ 时概率约 $0.58$，尚未达到完全导通；$1.00\%$ 时概率约 $0.87$，接近 $90\%$ 阈值。工程上若追求较小材料用量，可在 $0.70\%$--$1.00\%$ 区间增加实验；若追求确定性裕量，选择 $1.10\%$ 或更高更稳健。
...
\subsubsection{粗网格结果}
图~\ref{fig:q3scan} 展示阈值扫描结果。在 $M=2000$ 次独立试验下，体积分数 $1.00\%$（$707$ 根 A）时点估计 $\hat p=0.8755$（Wilson 下界 $0.8603$），$1.05\%$ 时 $\hat p=0.909$（Wilson 下界 $0.8956$），$1.075\%$（$760$ 根 A）时 $\hat p=0.9085$（Wilson 下界 $0.8951$），均未达到 $0.90$ 的 Wilson 认证标准。$1.08125\%$（$765$ 根 A）时 $\hat p=0.9195$，Wilson 下界 $0.9068>0.90$，首次通过统计认证。因此最低认证体积分数为 $1.08\%$，对应约 $765$ 根 A。

需要说明的是，问题二在 $1.00\%$ 处的点估计为 $0.8730$，与本节 $0.8755$ 略有差异：两者来自相互独立的随机数流（问题三扫描使用独立种子序列），差异 $0.0025$ 远小于 Wilson 区间半宽（约 $0.015$），属于正常抽样波动，两个问题的结论在统计意义上完全一致。
...
\begin{table}[H]\centering\caption{问题三阈值判定（$M=2000$）}\begin{tabular}{ccccc}\toprule 候选体积分数&A数量&成功次数&点估计&Wilson下界\\\midrule 1.00\%&707&1751/2000&0.8755&0.8603\\1.05\%&743&1818/2000&0.9090&0.8956\\1.075\%&760&1817/2000&0.9085&0.8951\\1.08125\%&765&1839/2000&0.9195&0.9068\\1.10\%&778&1856/2000&0.9280&0.9158\\1.20\%&849&1940/2000&0.9700&0.9616\\\bottomrule\end{tabular}\end{table}

\subsubsection{工程解释}
由式~\eqref{eq:fss}，阈值附近的概率曲线斜率很大，体积分数每增加 0.1 个百分点可能带来明显概率提升。实际设计不应只看单次仿真的点估计，还应考虑置信下界、加工误差和介质尺寸误差。在 $M=2000$ 的统计精度下，$1.08\%$ 为满足 $90\%$ 概率要求的最低认证填充量；若追求更大裕量，$1.10\%$（$92.8\%$）或 $1.20\%$（$97.0\%$）是更保守的制造方案。
...
由于决策空间为二维（$N_A$ 和 $N_B$），且每次函数求值成本较高（蒙特卡洛仿真），本文采用网格搜索策略\cite{quintanilla2017}。A 体积分数以 0.1 个百分点为步长（$0$--$1.0\%$），B 以 0.2 个百分点为步长（$0$--$1.6\%$），共 $98$ 个配置。每个网格点换算为整数数量后运行 $M=200$ 次蒙特卡洛试验。
...
\begin{table}[H]\centering\caption{问题四低成本候选方案（$M=200$）}\label{tab:q4candidates}\begin{tabular}{ccccccc}\toprule $\phi_A$&$\phi_B$&$N_A$&$N_B$&$\hat p$&Wilson下界&成本/元\\\midrule 0.70\%&1.20\%&495&358&0.9350&0.8920&7.948\\0.70\%&1.60\%&495&477&0.9600&0.9231&8.147\\0.80\%&1.20\%&566&358&0.9550&0.9167&9.002\\0.80\%&1.60\%&566&477&0.9750&0.9428&9.201\\1.00\%&0.40\%&707&119&0.9400&0.8981&10.694\\\bottomrule\end{tabular}\end{table}

\subsubsection{成本结构分析}
A 的单位体积价格是 B 的 21 倍，但 A 的长圆柱形状具有较强的跨空间连接能力；B 单体体积较大、价格低，单独使用时难以形成跨越微构体的连通路径（网格中纯 B 上限 $477$ 个仅 $\hat p=0.300$），而纯 A 需约 $765$ 根（成本约 $11.4$ 元）才能通过 $90\%$ 认证。混合方案显著优于两种纯组分：固定 $N_A=495$（$\hat p$ 仅 $0.555$），加入 $358$ 个 B 后 $\hat p$ 升至 $0.935$，成本仅增加 $0.60$ 元。点估计最低成本可行方案为 $495$ 根 A 与 $358$ 个 B（成本 $7.95$ 元）；若采用更严格的 Wilson 下界 $\ge 0.90$ 认证标准，最低成本方案为 $495$ 根 A 与 $477$ 个 B（下限 $0.9231$、成本 $8.15$ 元），较纯 A 方案节省约 $28\%$ 成本。这一发现与多组分渗流理论的预测一致\cite{maiti2018}：不同长径比介质的混合能通过交叉排除体积效应降低总体渗流阈值。
...
\subsection{数值一致性}
\begin{table}[H]\centering\caption{论文数值与结果文件核对}\begin{tabular}{p{5cm}p{4cm}p{4cm}}\toprule 核对项&论文值&来源\\\midrule 组1数量与导通&12、否&q1\_deterministic.csv\\ 0.50\%概率&0.3145&q2\_probability.json\\ 0.70\%概率&0.5825&q2\_probability.json\\ Q3认证阈值&1.08\%（765根）&粗扫描+二分+Wilson认证\\ Q4点估计最优&7.948元&495A+358B（M=200）\\ Q4 Wilson认证最优&8.147元&495A+477B（M=200）\\\bottomrule\end{tabular}\end{table}
...
\begin{table}[H]\centering\caption{问题二概率结果（$M=2000$）}\begin{tabular}{ccccc}\toprule $\phi_A$&$N_A$&$K/M$&$\hat p$&Wilson区间\\\midrule 0.50\%&354&629/2000&0.3145&[0.2945,0.3352]\\0.55\%&389&766/2000&0.3830&[0.3619,0.4045]\\0.60\%&424&895/2000&0.4475&[0.4258,0.4694]\\0.65\%&460&1033/2000&0.5165&[0.4946,0.5383]\\0.70\%&495&1165/2000&0.5825&[0.5607,0.6039]\\0.75\%&531&1272/2000&0.6360&[0.6147,0.6568]\\0.80\%&566&1386/2000&0.6930&[0.6724,0.7128]\\0.85\%&601&1476/2000&0.7380&[0.7183,0.7568]\\0.90\%&637&1579/2000&0.7895&[0.7711,0.8068]\\0.95\%&672&1680/2000&0.8400&[0.8233,0.8554]\\1.00\%&707&1746/2000&0.8730&[0.8577,0.8869]\\\bottomrule\end{tabular}\end{table}

\begin{table}[H]\centering\caption{问题四低成本候选方案（$M=200$）}\begin{tabular}{ccccccc}\toprule $\phi_A$&$\phi_B$&$N_A$&$N_B$&$\hat p$&Wilson下界&成本/元\\\midrule 0.70\%&1.20\%&495&358&0.9350&0.8920&7.948\\0.70\%&1.60\%&495&477&0.9600&0.9231&8.147\\0.80\%&1.20\%&566&358&0.9550&0.9167&9.002\\0.80\%&1.60\%&566&477&0.9750&0.9428&9.201\\1.00\%&0.00\%&707&0&0.8900&0.8391&10.495\\\bottomrule\end{tabular}\end{table}
