...
\begin{abstract}
针对边长 $10000\nm$ 微构体中的导电介质填充与跨电极导通问题，本文建立几何连通与随机优化模型。问题一中，读取附件三组 A 介质端点坐标，恢复圆柱轴线并构造介质—电极连通图，判断确定性导通状态。问题二中，将四个体积分数换算为 354、424、495、707 根 A，采用共同随机样本进行 4000 次共同随机数蒙特卡洛试验，得到导通概率分别为 0.0748、0.2035、0.4520、0.9845，并给出 Wilson 区间。问题三中，以首次导通所需数量的 90\\% 分位数定位阈值并用 Wilson 下限认证，得到最低认证体积分数约为 0.87\\%。问题四中，利用概率单调性进行成本排序和二分搜索，得到 580 根 A、115 个 B、概率 0.9140、成本 8.9876 元的方案。最后，本文通过边界处理、样本量和共同随机数敏感性分析检验模型稳定性，并提供可复现代码、结果文件和图表。
\end{abstract}
