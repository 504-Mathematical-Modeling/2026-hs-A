...
针对问题三，采用粗网格扫描与二分搜索相结合的策略，以 Wilson 下限作为统计认证标准，得到最低认证体积分数约为 $1.08\%$，对应约 $764$ 根 A。
...
\begin{table}[H]\centering\caption{问题二概率结果（$M=2000$）}\label{tab:q2result}\begin{tabular}{ccccc}\toprule $\phi_A$&$N_A$&$K/M$&$\hat p$&Wilson区间\\\midrule 0.50\%&354&629/2000&0.3145&[0.2945,0.3352]\\0.60\%&424&895/2000&0.4475&[0.4258,0.4694]\\0.70\%&495&1165/2000&0.5825&[0.5607,0.6039]\\1.00\%&707&1746/2000&0.8730&[0.8577,0.8869]\\\bottomrule\end{tabular}\end{table}
...
\subsubsection{机制分析}
从渗流理论角度，增加 $N_A$ 同时增大了三个效应\cite{berhan2007}：(i) 电极附近节点密度增加，增大了"种子"连通分量的起始规模；(ii) 介质间排除体积重叠概率增大，更多孤立团簇通过新边合并；(iii) 当 $N$ 足够大时，最大连通分量的相对尺寸从零突变到非零值——这正是渗流相变的特征。本题中 $\phi_A=0.50\%$ 时 $\hat p=0.3145$ 处于亚临界区，$\phi_A=1.00\%$ 时 $\hat p=0.8730$ 已接近超临界区，概率曲线的 S 形形态与三维渗流标度律一致。
...
\subsubsection{粗网格结果}
图~\ref{fig:q3scan} 展示阈值扫描结果。在 $M=2000$ 次独立试验下，体积分数 $1.00\%$ 时点估计 $\hat p=0.880$（Wilson 下界 $0.865$），$1.05\%$ 时 $\hat p=0.893$（Wilson 下界 $0.879$），均未达到 $0.90$。$1.075\%$（$760$ 根 A）时 $\hat p=0.916$，Wilson 下界 $0.903>0.90$，首次通过统计认证。二分精化后确定最低认证体积分数为 $1.08\%$，报告为 $1.08\%$。
...
\begin{table}[H]\centering\caption{问题三阈值判定（$M=2000$）}\begin{tabular}{ccccc}\toprule 候选体积分数&A数量&成功次数&点估计&Wilson下界\\\midrule 1.00\%&707&1760/2000&0.8800&0.8650\\1.05\%&743&1786/2000&0.8930&0.8787\\1.075\%&760&1832/2000&0.9160&0.9030\\1.10\%&778&1852/2000&0.9260&0.9137\\1.20\%&849&1930/2000&0.9650&0.9560\\\bottomrule\end{tabular}\end{table}

\subsubsection{工程解释}
由式~\eqref{eq:fss}，阈值附近的概率曲线斜率很大，体积分数每增加 0.1 个百分点可能带来明显概率提升。实际设计不应只看单次仿真的点估计，还应考虑置信下界、加工误差和介质尺寸误差。在 $M=2000$ 的统计精度下，$1.08\%$ 为满足 $90\%$ 概率要求的最低认证填充量；若追求更大裕量，$1.10\%$ 或 $1.20\%$ 是更保守的制造方案。
...
\subsection{数值一致性}
\begin{table}[H]\centering\caption{论文数值与结果文件核对}\begin{tabular}{p{5cm}p{4cm}p{4cm}}\toprule 核对项&论文值&来源\\\midrule 组1数量与导通&12、否&q1\_deterministic.csv\\ 0.50\%概率&0.3145&q2\_probability.json\\ 0.70\%概率&0.5825&q2\_probability.json\\ Q3认证阈值&1.08\%&粗扫描+二分+Wilson认证\\ Q4方案&4.4002元&283A+119B 复核\\\bottomrule\end{tabular}\end{table}
